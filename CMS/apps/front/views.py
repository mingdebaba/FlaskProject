...
from flask_paginate import Pagination#新增
...
def news_list():
    ...
    if not cat_id:
        ...
        cat_id=int(cat_id)
    #取得总的文章数
    total=db.session.query(Articles).count()
    list = []
    data = {}
    if request.method == 'GET':
        nav = Articles_Cat.query.all()
    ...
    cat1={}
    if cat_id!=0:
        cat1=Articles_Cat.query.filter(Articles_Cat.cat_id==cat_id).first()
        list1.append(cat1)
        cat1=Articles_Cat.query.filter(Articles_Cat.cat_id==cat1.parent_id).first()
        if cat1!=None:
            list1.append(cat1)
    ...
    # 新闻列表
    if not cat_id:
        pagination = Articles.query.order_by(Articles.create_time.desc()).paginate(page, per_page, False)
        news1 = pagination.items
        return render_template('front/news_list.html', cat=zz, pagination=pagination, news1=news1,cat_id=cat_id,news4=news4,list1=list2)
    else:
        pagination = Articles.query.filter(Articles.cat_id ==cat_id).paginate(page, per_page, False)
        news1 = pagination.items       
        return render_template('front/news_list.html', cat=zz, pagination=pagination, news1=news1,cat_id=cat_id,news4=news4,list1=list2)
#搜索页面路由
@bp.route('/search')
...
@bp.route('/case')
def case():
    return render_template('front/case.html')
#关于我们
@bp.route('/about')
def about():
...
#             logging.info(query.duration)
#
#     return response
#创建本地txt文件
# def text_create(name, msg):
#     path = os.getcwd()+'\\'#取得当前地址，\\表示一个\
#     full_path = path + name + '.txt'
#     file = open(full_path,'a+')
#     file.write(msg)
#     file.close()

#@bp.after_app_request    #测试完毕应该关闭
def after_request(response):
...
def test(id):
    print('url: %s , script_root: %s , path: %s , base_url: %s , url_root : %s' % (request.url,request.script_root, request.path,request.base_url,request.host))

    return 'sucess'

@bp.route('/article_detail/<int:id>/')
...
def getcode():
    ...
    mc.add('key', 'zhangshan', time=300)  # 一次只能设置一个值
    key = mc.get('key')  # 获取数据
    print(key)
    return 'sucess'
