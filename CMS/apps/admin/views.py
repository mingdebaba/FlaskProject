#encoding:utf-8
from flask import Blueprint,render_template,request,session,redirect,url_for,make_response,jsonify,json,flash
from .models import Users,Articles_Cat,Articles_Tag,Articles,Comment,Operate_Log,Admin_Log,Auth,Role
from .forms import LoginForm,Article_cat,Article,Checek_Auth,Checek_Role,RegisterForm
...
def index():
    ...
    for i in auths:
        data={
            'url' : "http://" + request.host + i.url,
            'name':i.name
        }

        list.append(data)

    return render_template('admin/index_new.html',auths=list,result=result)
#调用验证码
@bp.route('/code/')
def get_code():
    ...
    if mc.get('image')==None:#如果images键中没有数据，就执行添加操作
...
def article_edit_save():
    ...
    if request.method == 'POST':
        ...
        if form.validate():
            ...
            db.session.commit()
            return redirect(url_for('admin.article_list'))  # 新修改
        else:
...
#添加权限
@bp.route('/add_auth/',methods=['GET','POST'])
def add_auth():
    if request.method=='GET':
        auth1=Auth(
            name='查看文章列表',
...
def add_role():
    ...
    return 'sucess'
#测试权限
@bp.route('/test_role/',methods=['GET','POST'])
def test_role():
    user_id=session.get(config.ADMIN_USER_ID)
    ...
    print(auths_list3)
    #开始取得内容管理下的所有权限
    content_auths=db.session.query(Auth).filter(Auth.parent_id==1).all()
    for i in content_auths:
        for v in auths_list3:
            if v==i.url:
                authszz.append(i.url)
    print("**********")
    print(authszz)

    # rule = request.url_rule
    rule='/admin/admin_log_list/'
    print(rule)
...
@login_required
def admin_list():
    admins=db.session.query(Users.uid,Users.username,Users.email,Users.status,Users.role_id,Users.telephone,Users.remarks,Users.sex,Users.reg_time,Role.name).join(Role).filter(Users.role_id==Role.id).all()

    return render_template('admin/admin_list.html',admins=admins)
#修改管理员账号
@bp.route('/admin_edit_admin/',methods=['GET', 'POST'])
def admin_edit_admin():
    if request.method == 'GET':
        ...
        if id:
            global admin
            admin=Users.query.filter_by(uid=id).first()
        return render_template('admin/admin_edit_admin.html', admin=admin,roles=roles)
    else:
        #表单验证
        ...
        if forms.validate():
            ...
            if password==123456:#管理员没有修改密码
                Users.query.filter(Users.uid==uid).update({Users.username: username,Users.sex: sex,Users.telephone:telephone,Users.email:email})
                db.session.commit()
            else:#管理员修改了密码
                password = generate_password_hash(password)  # 密码加密处理
                Users.query.filter(Users.uid==uid).update({Users.username: username,Users._password:password,Users.remarks:remarks,Users.role_id:role_id,Users.sex: sex,Users.telephone:telephone,Users.email:email})
                db.session.commit()
        #表单验证不通过
        else:
            errors = forms.errors
            ...
            flash(errors, "error")#消息闪现
            return redirect(url_for('admin.admin_edit_admin'))
        data = {
            "msg": "修改成功",
            "success": 1,
        }
...
#添加管理员
@bp.route('/admin_add_admin/',methods=['GET', 'POST'])
@login_required
def admin_add_admin():
    if request.method=='GET':
        #获取角色
        roles = Role.query.all()
        return render_template('admin/admin_add_admin.html',roles=roles)
    else:
        ...
        return redirect(url_for('admin.admin_list'))
#管理员删除
@bp.route('admin_del_admin/',methods=['POST'])
@login_required
def admin_del_admin():
        id = int(request.values.get('id'))
        admin=db.session.query(Users).filter_by(uid=id).first()
        ...
        return jsonify(data)
#管理员停用
@bp.route('admin_stop_admin/',methods=['POST'])
@login_required
def admin_stop_admin():
        id = int(request.values.get('aid'))
        db.session.query(Users).filter_by(uid=id).update({Users.status: -1})
        #记录该操作，生成日志
        user_id = session.get(config.ADMIN_USER_ID)
        oplog = Operate_Log(
            admin_id=user_id,
            ip=request.remote_addr,
            operate="id为:"+str(id) +"的管理员账号被停用！"
        )
        db.session.add(oplog)
        db.session.commit()
        # 记录该操作，生成日志完毕
        ...
        return jsonify(data)
#管理员启用
@bp.route('admin_start_admin/',methods=['POST'])
@login_required
def admin_start_admin():
        id = int(request.values.get('aid'))
        db.session.query(Users).filter_by(uid=id).update({Users.status: 0})
        #记录该操作，生成日志
        user_id = session.get(config.ADMIN_USER_ID)
        oplog = Operate_Log(
            admin_id=user_id,
            ip=request.remote_addr,
            operate="id为:"+str(id) +"的管理员账号被启用！"
        )
        db.session.add(oplog)
        db.session.commit()
        # 记录该操作，生成日志完毕
        ...
        return jsonify(data)
#权限目录树
@bp.route('/admin_test_permission/',methods=['GET','POST'])
def admin_test_permission():
    if request.method=='GET':
        auths =Auth.query.order_by(Auth.id.desc()).all()  # 取得所有权限分类
        list = []
        data = {}
        ...
        data = build_auth_tree(list, 0, 0)
        html = build_auth_table(data, parent_title='顶级菜单')
        html1=creat_auth_table(data,parent_title='顶级菜单')
        print(html1)
